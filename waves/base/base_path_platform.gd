...
func _render_line():
	...
	while next_offset < line.curve.get_baked_length():
		...
		var node = place_node2(xform, next_offset + offset)
		line.add_child(node)
		nodes.push_back(node)
		next_offset += line.curve.bake_interval
	#var points = line.curve.tessellate_even_length(8, line.curve.bake_interval)
...
@abstract func place_node2(xform: Transform2D, node_offset: float) -> Node2D
